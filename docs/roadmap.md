...
> 当前版本 v0.21.0 — 体验打磨：徽章改为「成就小猫」、首页降密、删除番茄钟设置、修复首屏加载失败。

## v0.21.0 已上线（体验打磨）

四件事：

- **🐱 徽章 → 成就小猫**：旧徽章列表（积少成多 / 稳扎稳打 / 百时备考...）几个名字+icon 都相似，看起来像重复。本版重做成 11 个独立猫品种，每个对应一个明确的成就，稀有度跟难度挂钩：
  - common：中华田园猫（首次打卡）、三花橘猫（10h 累计）
  - rare：美短花纹猫（7 日连签）、暹罗猫（50h）、灰豹纹猫（单日 4h）
  - epic：布偶猫（30 日连签）、缅因猫（100h）、雪豹猫（单科 50h）、**金渐层（6 科各 ≥ 1 分钟）**
  - legendary：黑豹（300h 累计）、银渐层（6 科各 ≥ 10h）
  - 徽章页按稀有度分组陈列；顶部加可折叠的「成就说明」面板，每行写明「完成 X → 解锁 Y 小猫」
  - 服务端 `Badge` 类型新增 `rarity` 字段；客户端 tile 按品种稀有度上 4 种渐变 + 史诗加 🌟、传说加 👑

- **🐌 首屏「加载失败」修复**：WeChat 云托管 Node 容器冷启动有时超 7s，旧重试预算刚好不够。本版把冷启动重试节奏延长到 [0.5, 1.5, 2.5, 3.5, 5.5] = ~13.5s；首页 onShow 第一次走静默路径，失败后 2.5s 自动后台再试一次，**完全不弹 toast**，第二次才显示提示。冷启动场景下用户基本看不到红 toast 了。

- **⚙️ 删除番茄钟设置**：v0.18.1 已经把计时器锁定在 free 模式，但「我的 · 学习设置」里还保留 4 行番茄钟参数 stepper，纯噪音。本版整段移除，设置页只剩「每日目标 / 每周目标 / 学习音景 / 每日提醒」四块。底层 settings 对象的 pomodoro 字段保留向前兼容。

- **📐 首页降密（保留所有信息）**：减小卡片 padding / gap、收紧 stagger 动效延迟、稍微缩小巨大数字（倒计时 84→72rpx、计时器 108→96rpx）。卡片之间留白从 24rpx→18rpx，画面整体更紧凑，但每张卡片仍然存在、信息无丢失。

测试：188/188 全过；typecheck server + miniprogram 双绿。
...
- **去掉首页的「自由计时 / 番茄钟」切换**：用户反映两种模式样式过于接近、容易混淆。统一为单一"自由计时"模式（CPA 用户多为 1-2h 长 session，番茄严格 25/5 节奏并不贴合）。pomodoro 状态机代码保留以兼容老的进行中 session，但不再有入口能新建番茄 session。
- **选科目逻辑合并**：原先「开始前」+「结束打卡时」两处都让用户挑科目，过于繁琐且误导。改为只在首页"开始前"chip 选一次；完成打卡页改为只读 chip 显示「已选科目」+ 「如需修改请回到首页重新开始」hint。
- **头像绿色色块修复**：原先 `换头像/设头像` 是横跨头像底部 30% 的整条带，盖住面部下半部分。改为右下角 38rpx 圆形小 📷 角标（白边 + mint 渐变），不再遮挡头像图，全区域 tap 仍可触发 `chooseAvatar`。
- **首页倒计时升级回 hero 卡片**：v0.13 把考试倒计时从大卡瘦成 slim pill 是反向，用户反馈"太不明显"。重新升级为：考试日期 + 主科目 + 一句动机话 + 右侧 84rpx 大号天数 badge。urgency tier（calm / soon / urgent / imminent）继续驱动配色 + 30 天内 badge pulse 动效。
- **六科进度搬到首页**：作为 mini 卡片插在「计时卡片」与「今日目标」之间，6 行 横条 + 学时/目标。按降序排列（最常学的在最上），点击全卡进入完整六科页。利用既有 `/api/me/dashboard` 接口（与 `/home` 并行请求，互不阻塞），零后端改动。

测试：134/134 全部通过；typecheck server + miniprogram 双绿。

...
> ✅ **迁移已完成（2026-07-10，v0.45.0）**：微信云托管已整体下线，全部数据
> （239 用户、84 张打卡照片、11 张头像）已迁移至 VPS MySQL + 腾讯云 COS。
> 以下「共享库双跑」「让云托管续命」「回滚到云托管」等章节仅作历史记录，
> **现在不要再照着执行** —— 云托管环境已不存在。当前部署方式见
> [README.md](../README.md) 与 [RELEASE.md](../RELEASE.md)。
...
**硬约束**：已发布的旧客户端只会 `wx.cloud.callContainer`（云托管），物理上连不到 VPS。要让旧版本不中断，唯一办法是让云托管在过渡期继续活着；用户经微信自动更新换到新客户端后再停云托管。直接停云托管 = 旧版本立刻不可用。

**防数据分裂（split-brain）**：过渡期新旧客户端会同时在线。若云托管和 VPS 各连各的库，同一用户的读写会落到两个库、无法合并。**解法 = 两端共享同一个数据库**（纯环境变量，代码不用改）。

### 切换序列

1. **备案 + 开 VPS + 建库**。数据库选**两端都能连**的：腾讯云 CDB（推荐），或 VPS 上的 MySQL 开放给云托管连接。
2. **数据搬到共享库**：`mysqldump` 云托管 MySQL → 导入新库（见下「数据迁移」）。
3. **让云托管也连共享库**：给云托管补一点费续命，把它的 `DATABASE_URL`/`MYSQL_*` 改成新库，重新部署最新 commit。此刻起，旧客户端（→云托管）和（即将上线的）新客户端（→VPS）读写**同一份数据**，不分裂。
4. **VPS 上线**（连同一个库，配好 `.env`，nginx+PM2+HTTPS）。
5. **体验版验证**：传体验版（`runtime.ts` `apiBaseUrl`→VPS 域名），真机把全流程跑通（开始→结束→提交、补录、复盘、上传照片、**登录**）。**线上版本不动**，旧用户无感。
6. **灰度发布**新客户端（微信「分阶段发布」10%→50%→100%）。灰度期新旧并存——因共享库，安全。
7. **排空旧版本**：靠微信自动更新 + 本版起的 `getUpdateManager` 强制更新，多数用户 1–2 天内换到新版。监控云托管流量趋近 0。
8. **停云托管**。完成。

### 维护窗口 / 强制升级开关（v0.42，env 驱动）

服务端 `GET /api/app-config`（免鉴权，客户端启动时拉取，fail-open）：
- `MAINTENANCE=1` → **双重生效**：
  - 客户端启动弹「服务维护中」（劝阻新进入）；
  - **服务端硬冻结写入**——对 `POST/PUT/PATCH/DELETE /api/*` 直接返回 `503 MAINTENANCE`（豁免 `GET` 读、`/api/auth/login`、`/admin/*`）。这才是「避免写到一半」的真保证：终态 `mysqldump` 增量同步时开启，确保没有新数据写进来。改完重启进程生效。
- `MIN_CLIENT_VERSION=0.42.0` → 低于此版本的新客户端弹「请更新」并触发 `applyUpdate`。
- `MAINTENANCE_MESSAGE="…"` → 自定义文案（同时作为 503 的 message）。
> 注意：这些只对**本版（v0.42）起**的客户端生效；救不了最老的旧包（旧包只能靠云托管续命 + 自动更新排空）。共享库双跑时，云托管也跑这套代码，所以连云托管的较新客户端也能收到升级提示。

### 回滚

- 新后端异常且**云托管仍在**（共享库期间）：发一版 `apiBaseUrl=""` 的客户端即可切回云托管，数据因共享库不丢。
- 云托管已停：无法回退，故第 5 步**体验版真机全流程**是上线前的硬门槛。

## 风险 / 注意

- 备案是最长卡点（初审 1-2 天 + 管局约 7-20 天），但**只阻塞「配合法域名 + 正式发布」这最后一步**，其余全部可并行：
  - 备案本来就需要先有国内服务器出授权码 → **先买轻量、立刻提交备案**，等待期同步做部署（IP 直连）、迁数据、云托管续命共享库。
  - **验证不需要备案**：开发者工具勾「不校验合法域名」；真机打开「调试模式」同样跳过域名/HTTPS 校验 → 体验版全流程真机验证可在备案完成前做完。
  - 备案通过当天：DNS 解析 → certbot 签证书 → 小程序后台加合法域名 → 提审。通过到上线 <1 天。
  - 首次备案审核期内，该域名不要可访问（未备案域名在国内服务器 80/443 本来也会被拦），用 IP 调试即可；证书可用 certbot DNS-01 提前签好（不需要 80/443 可达）。
- **借已备案域名是最快路径**：微信校验的是域名 ICP 备案状态，备案按主域名生效、子域名直接可用——手里若有任何已备案域名，拿 `api.<已备案域名>` 指向新服务器即可当天走通全流程（严格合规还应在腾讯云做「新增接入」，可后补）。
- ⚠️ 澄清：**香港/境外服务器免备案 ≠ 能正式上线**。香港轻量确实不用备案就能跑服务端，但微信小程序后台添加 request 合法域名时会校验域名 ICP 备案，未备案域名加不进白名单 → 正式版用户请求发不出去。香港方案只对「开发调试 / 体验版（调试模式）」有意义，正式发布绕不开备案（或借已备案域名）。
